//can refer this or original code

#include <stdio.h>
#define MAX 10

int n, m, total[MAX], avail[MAX], alloc[MAX][MAX], max[MAX][MAX], need[MAX][MAX];

void input() {
    printf("Enter processes & resource types: ");
    scanf("%d %d", &n, &m);
    printf("Enter total resources: ");
    for (int i = 0; i < m; i++) scanf("%d", &total[i]);
    printf("Enter allocated & max demand:");
    for (int i = 0; i < n; i++)
...
int is_safe() {
    int work[MAX], finish[MAX] = {0}, seq[MAX], count = 0;
    ...
    while (count < n) {
        ...
        if (!found) return printf("System is NOT in a safe state!\n"), 0;
    }
